 ...
 */
 class BelowView {
    ...
    public BelowView(@NonNull Context c, @Nullable List<String> listName) {
        this.convertView = View.inflate(c, chuangyuan.ycj.videolibrary.R.layout.simple_exo_belowview, null);
        listView = convertView.findViewById(chuangyuan.ycj.videolibrary.R.id.list_item);
        if (listName == null) {
            listName = Arrays.asList(c.getResources().getStringArray(chuangyuan.ycj.videolibrary.R.array.exo_video_switch_text));
        }
    ...
    public void showBelowView(@NonNull View view, boolean canceledOnTouchOutside, int selectIndex) {
        if (pw == null) {
            int height= (int) (view.getResources().getDimension(chuangyuan.ycj.videolibrary.R.dimen.dp30)* adapter.getCount());
